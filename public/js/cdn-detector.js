    ...
    constructor() {
        this.cdnSignatures = {
            ...
            'Akamai': {
                ...
                domains: ['akamai.net', 'akamaized.net', 'edgesuite.net', 'akamaitechnologies.com', 'akamaiedge.net'],
    ...
    async detectCDN(domain) {
        ...
        for (const [provider, signatures] of Object.entries(this.cdnSignatures)) {
            ...
            for (const server of signatures.servers) {
                if ('server' in data.headers && data.headers['server'].includes(server)) {
                    results.evidence.push({
                        provider,
                        weight: 30,
        ...
        if (results.evidence.length > 0) {
            ...
            const maxProvider = Object.keys(providerAndWeights).find(provider => providerAndWeights[provider] === maxWeight);
